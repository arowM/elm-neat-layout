...
For example, the `View` generated with the following code will be wrapped as follows.
...
In contrast, the `View` generated by the following code will be wrapped as follows.
...
-}
fromTexts : List (Text msg) -> View NoGap msg
fromTexts ls =
    TextNode
        { mixin = Mixin.none
        , layout =
...
textNode : Renderer_ -> { mixin : Mixin msg, layout : Layout, innerGap : Gap, overlays : Overlays msg, text : List (Text msg) } -> Html msg
...
croppedTextCore : Float -> Bool -> List (Text msg) -> Html msg
croppedTextCore lineHeight wrap texts =
    case List.filter (\t -> t.text /= "") texts of
        [] ->
            Html.text ""
        ...
        filtered ->
            Mixin.span
                ...
                (List.map (renderText wrap) filtered)


renderText : Bool -> InlineNode msg -> Html msg
renderText wrap text =
    Mixin.lift (Html.node text.nodeName)
        [ text.mixin
        , enforcedStyle
        , Mixin.unless wrap <|
            style "white-space" "nowrap"
